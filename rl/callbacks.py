...
class TrainEpisodeLogger(Callback):
    def __init__(self, savepath, interval=None):
    ...
    def on_episode_end(self, episode, logs):
        ...
        if np.sum(self.rewards[episode]) > self.lastreward:
            ...
            previousWeights = f'rew_{round(self.lastreward)}_epi_{episode + 1}.h5f'
            if os.path.exists(os.path.join(self.savepath, previousWeights)):
                os.remove(os.path.join(self.savepath, previousWeights))
            self.lastreward = np.sum(self.rewards[episode])
            print("The reward is higher than the best one, saving checkpoint weights")
            newWeights = f'rew_{round(np.sum(self.rewards[episode]))}_epi_{episode + 1}.h5f'
            self.model.save_weights(os.path.join(self.savepath, newWeights), overwrite=True)

        else:
            print("The reward is lower than the best one, checkpoint weights not updated")

        # save current model on given episode numbers
        if self.interval is not None and episode % self.interval == 0:
            self.model.save_weights(os.path.join(self.savepath, "latest_model.h5f"), overwrite=True)
